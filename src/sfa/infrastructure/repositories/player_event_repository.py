    ...
    async def get_fixtures_by_player(
        ...
        stmt = (
            select(
                ...
                away_alias.c.name.label("away_team"),
                Fixture.played_at,
                func.coalesce(func.sum(PlayerEvent.pts), 0).label("sfa_pts"),
                func.count(PlayerEvent.id).label("events_count"),
                PlayerStats.minutes,
                PlayerStats.duels_won,
                PlayerStats.dribbles_won,
            )
            ...
            .outerjoin(
                PlayerEvent,
                and_(
                    PlayerEvent.fixture_id == Fixture.id,
                    PlayerEvent.player_id == PlayerStats.player_id,
                ),
            )
