...
class IngestionRepository(IngestionRepositoryPort):
    ...
    async def upsert_player(
        ...
        stmt = (
            ...
            .values(
                ...
                position=position, photo_url=photo_url,
            )
            .on_conflict_do_update(
                index_elements=["external_id"],
                set_={"name": name, "team_id": team_id, "position": position, "photo_url": photo_url},
            )
            .returning(Player.id)
        )
        result = await self._session.execute(stmt)
    ...
    async def fixture_players_already_downloaded(self, fixture_id: int) -> bool:
        result = await self._session.execute(
            select(PlayerStats.id).where(PlayerStats.fixture_id == fixture_id).limit(1)
        )
